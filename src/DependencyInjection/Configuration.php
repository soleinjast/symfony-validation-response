...
final class Configuration implements ConfigurationInterface
{
    public function getConfigTreeBuilder(): TreeBuilder
    {
        ...
        $rootNode
            ...
            ->scalarNode('format')
            ->defaultValue('simple')
            ->info('Default error format: simple, nested, or rfc7807')
            ->validate()
            ->ifNotInArray(['simple', 'nested', 'rfc7807'])
            ->thenInvalid('Invalid format "%s". Must be either "simple", "nested", or "rfc7807"')
            ->end()
            ->end()
            ->arrayNode('rfc7807')
            ->addDefaultsIfNotSet()
